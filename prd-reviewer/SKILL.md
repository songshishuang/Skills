...
```
第一关 规则审查 lint      ~2 分钟/story   审「格式与可测性」(standard 8 项 / lite 5 项)
...
【standard lint · 8 项】(无 type 或 type ≠ lite)
...
7. source / prototype 为相对路径且**在 PM 仓内可解析(目标文件真实存在)**;
   未内嵌 HTML/截图大段内容
8. AC 引用的**验收依据**(golden fixture / 数据样例 / 期望值 / 枚举对照)其内容
   须在 00-overview.md 或本 story 内可解析;仅以"见 PRD §x"引用全本章节、内容不在
   stories/ 内 = blocker(研发只读 overview+story,读不到=AC 不可复跑)
...
- 对 lite story 用了 standard 8 项门禁(或反之)→ 门禁错配,重跑
...
- **2026-06-18 · v1.4** — 第一关 lint 加**第 8 项「AC 验收依据可解析」**(随 prd-writer v2.4.7 · stories 重派生二关回流)：AC 引用的 golden fixture / 期望值 / 枚举对照,内容须在 overview/story 内可解析;仅"见 PRD §x"而内容不进 stories/ = blocker(研发只读 overview+story 读不到=不可复跑)。standard 门禁 7→8 项,三处同步(总览行/lint prompt/Red Flags);golden 正样例 AC 不引用全本数据故仍过第 8 项(self-consistency 保持),负样例第 8 项探针待下轮补。
