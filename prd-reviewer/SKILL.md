...
| 全本 PRD(模式 B) | 按 prd-writer DoD 做 lint | 检查核心必填 9 项 + 永远不写 2 项,输出同格式报告 |
...
完成后逐条对照 AC 自验,输出验收对照表(AC / 实现位置 / 自验结果),
并截图关键页面。
```
...
全本 PRD 模式(模式 B):按 `prd-writer` SKILL.md 的「完成标准(DoD)」逐项 lint,核心必填 9 项缺任一 = blocker,出现「永远不写」内容 = blocker,输出同格式报告。
