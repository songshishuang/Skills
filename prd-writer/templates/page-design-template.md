...
> **用途**：PRD §4.2 「页面功能详细设计」每个页面的标准展示模板。开发知道"做什么"、QA 知道"验什么"、运营知道"怎么用"。
...
| **关键字段** | （数据字段清单 · 注明字段来源 = 数据模型上游收集字段 ⊆ §4.1 业务对象 · 字段闭环校验）|
...
- **数据字段清单** · 必须 ⊆ §4.1 业务对象字段（字段闭环校验 · 字段必须来自上游收集字段）
- **格式**：字段名 · 业务含义 · 取值范围（如有）
...
| 状态变化写「正常流转」 | 引用 §4.1.3 生命周期状态机或简短列状态迁移 |
