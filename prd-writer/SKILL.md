...
**配套交付 · 角色切片**：每次产出全本 PRD 后，自动派生 **stories/ 目录**（研发 AI 编码用：`00-overview.md` + 按 story 拆分的 `ST-XXXX` 卡片）+ 3 个角色切片（`for-qa` / `for-bi` / `for-ops`）—— 解决"一份 PRD 撑 6 个消费者互相污染"的问题。AI 编码 agent 每次只读 `00-overview.md` + 单个 story 卡片，上下文最小化。
