...
3. **必读 `docs/wiki/glossary.md`** —— 保持术语一致性
...
4. **回填 Stage 1 的 6 个核心问题**：原型通常能直接回答 4 个，仍需找用户确认剩余 2 个（优先级 + AI 模块 + 灰度 + 上线时间）
...
| 关键字段 | 数据字段清单 ⊆ §4.1 业务对象 · 反模式 25 校验 |
...
写完全本 PRD 后，**默认同时生成 4 个角色切片**：

| 切片 | 消费者 | 模板 |
|---|---|---|
| `slices/for-coding.md` | AI 编码 agent / 研发 | `templates/prd-template-coding-slice.md` |
...
- [ ] **Stage 0 完成**：如项目存在 `docs/wiki/`，已读 index.md + 相关页（如不存在则注明"项目无 wiki"）
...
| **手工编辑 slices/ 目录的切片** | 切片是派生产物，手工改会被下次重新生成覆盖 | 改全本 PRD，重新派生切片 |
...
| "7 节还是太多，合并一些" | 严禁。7 节是给所有消费者覆盖的最小集；切片机制已经解决"研发觉得多"的问题，让研发读 `for-coding.md` 即可 |
...
│   ├── prd-template-coding-slice.md        # ⭐ 编码切片模板（150 行内 · 仅业务）
...
└── references/
    ├── slice-generation.md                 # ⭐ 4 个角色切片的生成规则
...
| 用户拒绝某段写法（"不要这样写 / 删掉"） | 这条否定是不是普适规则？ | 反模式表（SKILL.md 内）或 `references/anti-patterns.md` |
...
关键演进（v2.3）：**不再追求"一份 PRD 满足所有人"，而是"一份全本 + 多个切片"**。AI 编码 agent 读 `for-coding.md`，QA 读 `for-qa.md`，BI 读 `for-bi.md`，SRE 读 `for-ops.md` —— 各取所需，互不干扰。
