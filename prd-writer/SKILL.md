...
**⭐ 回溯护栏（功能已实现 / 已上线时反推）**：原型未建时反推=前瞻需求；**功能若已合并/上线，反推极易退化成 release note**。守三条：① frontmatter `status` **诚实反映本 PRD 用途**——"待评审需求"用 `Draft`/`In-Review`，"上线后追溯记录"才用 `Released`，**两者别混**（评审人要能一眼分清）；② 实现史（commit 列表 / 缓存版本传播等）**压到最简、归 §7 附录**，不进 frontmatter 顶部与正文；③ 正文仍按 **问题 → 决策依据 → 验收口径** 组织，不是"做了什么"的流水账。
...
| 4.1.2 关系图 | mermaid erDiagram · **不写 PK/FK/类型** · 用业务概念命名 · **标重数(1:1 / 1:N)**；尤其追问"该对象是否会多版本 / 重测 / 历史多次"（如 Endpoint 可多次准入）——若是则**不是 1:1**，并在 4.1.4 定"展示最新 / 当前 / 全部"口径 |
...
### Stage 4：生成切片 + 附属交付物（自动）

写完全本 PRD 后，**默认同时生成 stories/ 目录 + 3 个角色切片 + 2 个附属交付物**。每个产物都有可照填的模板文件，照模板填、不要自创结构：
...
| `slices/for-qa.md` | QA / 算法 | `templates/slice-for-qa.md`（抽取规则见 `references/slice-generation.md`） |
| `slices/for-bi.md` | BI / 数据 | `templates/slice-for-bi.md` |
| `slices/for-ops.md` | SRE / 运营 | `templates/slice-for-ops.md` |
| `埋点表.csv` | BI 建表 | `templates/tracking-table-template.csv`（9 列口径见 `references/tracking-events-spec.md` 第二节） |
| `评测集要求.md` | QA / 算法 | `templates/eval-set-requirements.md`（涉及 AI 模块时必出） |
...
| **本期不做 / 仅登记的需求塞进 §1 核心变化或 §3 Scope** | 干扰评审范围理解、研发误判工作量（"以为本期有活"） | 移入 §2.6 Open Questions 或 §7 后续需求池；§1/§3 只放本期真做的 |
| **占位 / 未实现功能不定呈现态** | 埋点 / 交互 / QA 各做各的（disabled？toast？隐藏？） | 明确呈现态（禁用 / 隐藏 / 可点提示），且埋点与交互与该态一致 |
...
| **把一个页面/组件按 UI 层（框架 / 行 / 区域）水平拆成多卡** | 卡间互相依赖、谁都不能独立交付，造出现实里不存在的集成缝（"行怎么插进框架"）；prd-reviewer 第二关会判高风险 | 按**垂直价值增量**切（每片穿透 UI+逻辑+数据、独立可交付/验收）；一页通常=1 卡，可拆 MVP→增强等垂直增量，但切不出第二个独立价值块就别拆 |
...
| **story 正文点兄弟 story_id 描述对方职责** | 双向点名（ST-A↔ST-B）= 隐性耦合，改一个要改俩；依赖散落正文难维护 | 依赖只在 overview 索引表「依赖」列声明（单向集中）；story 正文「依赖与边界」只说本 story 自身边界，不点兄弟 ID |
