...
❌ **评审备注贴「上线前删除」标签后仍渲染进 UI**（2026-06-12 盲测抓获）：把机制说明做成红虚线「📋 评审备注 · 上线前删除」条渲染在页面上——贴了免责标签不改变"说明性文案进了渲染层"的事实。**§20「演示用状态切换器」先例仅适用于操作原型状态的功能控件**（切换器/状态下拉），不适用于任何说明性文案；说明文案的唯一合法去处是 HTML 注释（不渲染）/ PRD / story 卡片。
