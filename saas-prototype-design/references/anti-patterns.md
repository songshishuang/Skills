...
每条都来自 某 SaaS 平台 项目 30+ 轮迭代的真实返工。**避坑就是节省时间**。
...
**新旧字段对照 / 现有系统术语映射也属于此**：研发期为了打通现有系统，可能临时想在 UI 里加蓝色框「📘 与现有系统术语对齐：chatcompletions = chat 无 stream / functioncall = tool_use / opensisdk = openai 协议 ...」这种对照表。这是给开发看的，不是给用户看的。**对照表应放在迁移文档**（如 `docs/migration/field-mapping.md`），UI 上只显示新字段名 + 简短中文释义。

**page-header-desc 副标题塞描述性长句也属于此**：例如「全平台模型主体维护 · 1 个模型 ↔ N 个 Endpoint · 用于评测、上架、定价、监控的统一模型字典」——这是 PM/产品架构师对自己理解的描述，不是面向使用者的功能说明。**用户打开页面已经从菜单进来，不需要被告知"这是什么页面"**。极端情况留 1 句解释，否则只留标题。

**「v0.1 仅启用 / v1.0 即将启用 / 一期范围 / 完整规范在 v1.x」最高频踩坑**（截至 v1.8 已踩 4 次，每次清理 5-7 处）：

任何形如以下文案**全部禁止进入 UI**：
- 醒目横幅「💡 v0.1 范围：本期仅 X · Y 在 v1.0 启用」「🔒 一期范围：仅启用 G1（4 项）。G2-G5 在 v1.0 启用」
- 折叠区 `<details>📋 完整 P5.7 还有 4 组 21 项 · v1.0 启用</details>`
- page-header-desc 写「v0.1 仅启用 D1 基础能力 + D2 工程性能 · 推理模式 N0-N4」
- KPI 副标 / 卡片注脚写「v0.1 仅做 D1 基础能力 + D2 工程性能」
- 表格行底「D3-D7 v1.0 启用」、字段值写「v0.1 不评测多模态能力」
- toast 弹「该功能将在 v1.0 启用」（去掉版本号，改通用「该功能即将上线」）

| ❌ 错误 | ✅ 替代 |
|---|---|
| 范围 banner | **删除整个 banner**。功能没做就别在 UI 里"为不存在的功能道歉" |
| `<details>` 折叠展示完整规范 | **删除**。需要给评审看完整规范 → 放规划文档 (`docs/prd/`) 里链接 |
| 字段值「v0.1 不评测多模态」 | 改「暂不支持」/ 空 / `—`，让字段表达事实而非范围决策 |
| KPI 副标「v0.1 仅展示 D1+D2」 | 改中性叙述「D1 基础能力 + D2 工程性能 · 自动跑分」 |
| toast「v1.0 启用」 | 改「该功能即将上线」（去版本号，不暗示 v1.0 是何时） |

**为什么 PM 总想加？**
- PM 担心评审人问"为什么没有 X 功能"→ 想用 banner 提前解释
- 真相是：**评审人能从功能存在性判断范围**，banner 反而显得"心虚"+ 暴露半成品
- 范围说明的正确位置是 PRD / 规划文档 / release notes，**不是产品 UI**

**全局排查脚本**（每次新增页面 / 提交前必跑）：

```bash
grep -rE "v1\.0 启用|v0\.1 仅|一期范围|本期仅|完整 [A-Z][0-9]\.|<details" \
  prototypes/v0.1/*.html prototypes/v0.1/vendor/*.html 2>/dev/null \
  | grep -vE "key-features:|changelog:|<!--|\.history/"
# 任何匹配都是反模式 1 违例，必须删
```
...
<!-- 不要：<div class="page-header-desc">全平台模型主体维护 · 1 个模型 ↔ N 个 Endpoint · ...</div> -->

<!-- 新建表单修正 -->
<label>模型类型</label>      ← 中文直说
<!-- 不要：「📘 与现有系统术语对齐」蓝框对照表 -->
```
...
某 SaaS 平台 项目中 Agent C 任务是「复制 v1 文件到 v0.1 + 改 brand」，结果直接覆盖了之前 3 轮迭代精修的 `v0.1/eval-sets.html`（启用 toggle / 描述 / 版本历史 / 模型基线 3 sub-tab 等都丢了）。
