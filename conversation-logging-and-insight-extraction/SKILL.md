...
6. **算力成本溯源 (Token Tracking)**：
   - **会话级统计**：在单点记录的标题下方**单独起一行**，标注该轮对话的 Token 消耗量。通过本地代码（如 `tiktoken` 库）精确计算双向（User + AI）的 Token 数值，体现降本增效意识（格式：`*Token消耗: ~X*`）。
   - **大盘级统计**：在每日和每月的汇总文档顶部（日期/月份声明下方），必须汇总当期所有的 Token 消耗总量（格式：`**今日算力消耗**: ~X Tokens` 或 `**本月累计算力消耗**: ~Y Tokens`）。
