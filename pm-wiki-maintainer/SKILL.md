...
**wiki 根探测**：按 `docs/wiki/` → `docs/pm/wiki/` 顺序探测，取首个存在者记为 `{wiki_root}`（与 prd-writer Stage 0 同规则，防协作漂移）；两者皆无 → 走「骨架自举」（默认建 `docs/wiki/`，用户可指定其他位置）。**本 skill 全部文档中的 `docs/wiki/` 均指 `{wiki_root}`。**
...
│   └── {topic}.md    ← 各主题决策综合（如 llm-model.md 汇总所有跟 LLM 选型相关的 ADR；目录索引由根 index.md 的决策段承担）
│
├── topics/            ← 跨多文档的主题页
...
三类操作启动时先按上节规则探测 `{wiki_root}`；若**不存在或缺 `index.md`**：暂停原操作，问用户「项目尚无 wiki 骨架，用模板初始化？」。确认后投放本 skill 模板：`templates/INDEX.md → index.md`、`templates/LOG.md → log.md`、`templates/CLAUDE-wiki.md → CLAUDE.md`（`{{DATE}}` 替换为当日），并建 `concepts/ entities/ decisions/ topics/` 空目录与 `glossary.md`，随后继续原操作。

> **templates/ 三件是 wiki 骨架的唯一真相源**——其他 skill（如 pm-project-scaffolding）建骨架时应复制本目录模板，不要内联自造副本（防双源漂移）。
...
│    ├── decisions/<主题>.md     从 PRD §7.2 抽出关键决策，按主题归并（方案 ii）│
...
| `prd-writer` | PRD 评审通过后 | §7.2 ADR 中的关键决策 / §2.2 新增用户角色 / §3.1 新功能模块概念 / §4.3 AI 模块新维度 | **prd-writer Self-Evolving 时建议 → 用户触发 ingest** |
...
| Lint 发现的 contradiction 模式高频出现 | 是不是要进 anti-patterns 表？ | 届时新增独立反模式参考页（当前各 workflow 文件内的反模式节已覆盖，尚未单立文件） |
...
### 简化驱动评估（v1.3+ 新增 · 反向防止 wiki / references 单调膨胀）

**反向简化清单 · 每次 evolution 必问**：

- [ ] **reference 文件健康度**：4 份 workflow doc（ingest / query / lint / wiki-structure）有没有哪份 ≥ 3 个月没被实际工作流读到？标 deprecated 候选
- [ ] **frontmatter 字段裁剪**：wiki-structure.md 约定 6 个 frontmatter 字段（type / created / updated / sources / related / status——整体「强烈建议」，其中 ingest 改动页时 updated/sources 必加），实际有几个真被填了？没填的进一步降级
- [ ] **ingest 流程裁剪**：8 步 ingest 流程对小源（≤ 1 页会议纪要）是否过重？是否要分轻重档？
- [ ] **lint 6 类裁剪**：哪一类 lint 检查 ≥ 3 个月没新发现？降频或合并
- [ ] **Changelog 反向流**：最近 N 条 changelog 是不是全是「新增」？这次能不能落一条「合并 / 删除 / 降级」？
- [ ] **跨 skill 协作约定时效**：对其他 skill 章节号的引用（如 PRD §X.Y）是不是仍准确？过时引用会让协作断链
...
- **2026-06-12 · v1.4** — **营造（yingzao）大修 Y-009（盲评 58 起修 · 五轮细作）**：① decisions/ 页粒度矛盾收敛——统一为方案 ii 主题综合页（修复「每条 ADR 一页」漂移口径，3 处同向）；② 落地管线修复——新增「骨架自举」工作流（templates/ 三件从资源孤儿变为有投放管线，声明为骨架唯一真相源），templates/CLAUDE-wiki.md 三处跨 skill 四层相对路径死链改 runtime 无关引用 + 安装路径中立化；③ wiki 根探测式（`docs/wiki/` → `docs/pm/wiki/`，与 prd-writer Stage 0 同规则）；④ tests/ 测试资产从零建（4 条四件套 + 裸基线预期表），T2/T3 装载版隔离实测全判据通过；⑤ 一致性收口——档位 200-500 字缝隙判定、frontmatter 强度词统一、`_index.md` 声明移除、ingest 中断恢复、PRD 映射表节名对齐 v2.4
