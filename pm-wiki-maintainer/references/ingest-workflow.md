...
> **缝隙判定**（200-500 字之间）：含新决策或新概念 → 完整档；纯信息散布 → 轻档（轻档内置升档闸口兜底，判错可中途升）。
...
会话中断后重跑：先读 `log.md` 末条与 `index.md`——本次源已有 log 条目 = 已完成；有部分页面 frontmatter `sources` 含本源但无 log 条目 = 第 5-7 步间中断，从缺失步骤续作（先补漏页再补 index/log），**不重做已触达页**。
...
### 从 PRD ingest（约 6-10 页 wiki 改动 · 基于 prd-writer v2.3+ 7 节结构，v2.4 沿用）

| PRD 节 | 提炼到 wiki 的什么 | 目标页 |
|---|---|---|
| §2.3 业务目标（北极星 + KR） | 项目目标演进史 | `topics/目标-演进.md`（每个 PRD 版本一条记录） |
| §2.2 用户角色 | 新增角色 / 既有角色画像更新 | `entities/users/<role>.md` |
| §3.1 需求清单 | 新增功能模块 | `concepts/<模块>.md` |
| §4.3 AI 模块特化 | 模型选型 / 提示词版本 / 工具调用维度 | `concepts/ai-module-spec.md`（综合页） |
| §5.5 成本预算 | token 单价 + 月度预算演进 | `topics/成本-演进.md` |
| §7.2 ADR | **按主题归并进决策综合页**（方案 ii：跨 ADR 关联 + 过时标记），新 ADR 追加进所属主题页，标 active / superseded；无所属主题页时新建 | `decisions/{主题}.md`（如 `llm-model.md`） |
...
| 模块边界调整 | 哪些模块拆分 / 合并 / 移交下游 | `decisions/{主题}.md`（如 `架构边界.md`，归并进主题综合页） |
